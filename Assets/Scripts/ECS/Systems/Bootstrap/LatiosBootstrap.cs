...
using ECS.Systems.Movement.SuperSystems;
...
namespace ECS.Systems.Bootstrap
{
...
//            Latios.Kinemation.KinemationBootstrap.InstallKinemation(world);
  //          Latios.Calligraphics.CalligraphicsBootstrap.InstallCalligraphics(world);
            ...
            var fixedGroup = world.GetExistingSystemManaged<PhysicsSystemGroup>();
            var type = TypeManager.GetSystemTypeIndex<MovementSuperSystem>();
            BootstrapTools.InjectSystem(type, world, fixedGroup);

            type = TypeManager.GetSystemTypeIndex<ApplyConstraints>();
            BootstrapTools.InjectSystem(type, world, fixedGroup);
            
            type = TypeManager.GetSystemTypeIndex<PlayerMovement>();
            BootstrapTools.InjectSystem(type, world, fixedGroup);

            type = TypeManager.GetSystemTypeIndex<UpdatePlayerPosition>();
            BootstrapTools.InjectSystem(type, world, fixedGroup);

            type = TypeManager.GetSystemTypeIndex<FollowPlayer>();
            BootstrapTools.InjectSystem(type, world, fixedGroup);
